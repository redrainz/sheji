...
class SprintStore {
  ...
  setCurrentEditData(currentEditData) {
    if (currentEditData.issueType === 'task') {
      let id = currentEditData.parentId;
      axios
        .get(`/kanban/v1/issue/${id}`)
        .then(data => {
          if (data) {
            this.setParentName(data.description);
          }
        })
        .catch(err => {
    ...
    this.taskType = this.currentEditData.issueType === 'task';
