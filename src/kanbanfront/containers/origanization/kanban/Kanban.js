...
class Kanban extends Component {
  ...
  resizeKanbanContent = () => {
    ...
    let height = Number(
      autoRouter.style.height.substr(0, autoRouter.style.height.length - 2),
    );
    ...
    for (let z = 0; z < allColumn.length; z++) {
      ...
      if (this.swimLaneArea[column.id].length === 0) {
        ...
        for (let k = 0; k < tempSwimLine.length; k++) {
          ...
          let tempheight =
            nextItem !== undefined
              ? nextItem.y - item.y - item.height
              : kanbanHeight - item.y - item.height;
          let temp = {
            height: tempheight,
            width: column.width,
            y: tempy,
  ...
  initTable() {
    ...
    for (let item of document
      .getElementsByClassName('kanban-table')[0]
    ...
    if (cards !== undefined && cards[`${columnId}`] !== undefined) {
      ...
      area[columnId].map(item => {
        for (let i = 0; i < cards[`${columnId}`].length; i += 1) {
          ...
          if (
            item.y <= card.positionY &&
            card.positionY < item.y + item.height &&
            item.y === y &&
            x === card.positionX
          ) {
            if (i === 0) {
      ...
      if (swimlane.length !== 0 && swimlane.y === y) {
        for (let l = 0; l < columns[index].width; l++) {
          tempCells.push(
            <div>
              <Droppable
                droppableId={`${columns[index].id},${y},${l},${
                  swimlane.id
                },swimLane`}
              >
                {(provided, snapshot) => (
  ...
  generateTable(array, depth, kanbanheight) {
    ...
    let currentLayoutLength = array.length,
      nextLayoutLength = 0,
      currentLayout = 1;
    ...
    while (queue.length !== 0) {
      ...
      row.push(
        <td
          id={node.id}
          rowSpan={
            node.subColumns === null || node.subColumns.length === 0
              ? depth - currentLayout + 1
              : 1
          }
          colSpan={node.colSpan}
          key={node.id}
          data-colWidth={node.width === 0 ? 1 : node.width}
          data-status="doing"
          data-columnId={node.id}
          data-nextToBody={node.nextToBody}
          data-position={node.position}
        >
          <SystemColumnContent
            columnStatus={node.status}
          ...
          >
            {node.wipNum != -1 && cardNum != null && node.wipNum != null
              ? `${cardNum}/${node.wipNum}`
              : ``}
          </div>
        </td>,
      ...
      if (node.subColumns != null) {
        /*若子列数组不为空，则依次将子列添加到队列中，并记录下层节点数量*/
        nextLayoutLength += node.subColumns.length;
        for (let item of node.subColumns) {
          queue.unshift(item);
        }
      } else {
        /*防止子列数组出现null，引起不必要的异常*/
        node.subColumns = [];
      }
      if (currentLayoutLength == 0) {
        /*当前层次下节点的数量为0时，进入下一层，将row包装成tr压入thead*/
      ...
      for (let j = 0; j < this.allColumn.length; j += 1) {
        ...
        if (z === kanbanheight - 1) {
          rowTbody.push(
            <td
              style={{
                minWidth: 163 * this.allColumn[j].width,
                borderTop: 'none',
              }}
            >
              {this.getCell(
    ...
    for (let j = index[0]; j < index[0] + tempArea.width; j++) {
      if (this.allColumn[j].card != null) {
        this.allColumn[j].card.map(card => {
          if (card != null) {
            if (
              card.positionY >= tempArea.y &&
              card.positionY < tempArea.y + tempArea.height
            ) {
  ...
  getCardCountInCell = (
    ...
    cards.map(card => {
      if (
        card.positionY >= area.y &&
        card.positionY < area.y + area.height &&
        card.positionY === Number(y)
      ) {
        xcount += 1;
      }
      if (
        card.positionY >= area.y &&
        card.positionY < area.y + area.height &&
        card.positionX === Number(x)
      ) {
        ycount += 1;
      }
    });
    let limit =
      area.columnWidth == null
        ? area.width * area.height
        : area.columnWidth * area.height;
    ...
    if (result.destination != null) {
      ...
      if (endArray[0] !== 'temp') {
        ...
        cards[endArray[0]].map(item => {
          if (
            item.positionY >= tempArea.y &&
            item.positionY < tempArea.y + tempArea.height
          ) {
            countCard.push(item);
          }
        });
      ...
      if (startArray[4] === 'CardChild') {
            ...
            for (let h = 0; h < card.subIssue.length; h++) {
              ...
              if (item.id === result.draggableId) {
                ...
                KanbanStore.updateIssueById(item.id, item).then(res => {
                  if (res) {
                    KanbanStore.getCardById(
                      this.props.match.params.kanbanId,
                    ).then(response => {
                      if (response) {
                        this.setState({
                          cards: response,
        ...
        countCard.length < tempArea.height * tempArea.width &&
        endArray[4] !== 'swimLane'
      ) {
        console.log('进入area');
        let cardCountInCell = this.getCardCountInCell(
          tempArea,
          endArray[0],
          startArray[0],
          endArray[1],
        ...
        let tempId = this.getEmptyCell(
          tempArea,
          endArray[0],
          tempArea.height * tempArea.columnWidth,
        );
        let cardCountInCell = this.getCardCountInCell(
          tempArea,
          endArray[0],
  ...
  handleScroll = e => {
    ...
    KanbantheadDom[0].setAttribute(
      'style',
      `top:${KanbanContentDom[0].scrollTop}px;position:absolute;z-index:7;`,
    );
  };
  ...
  render() {
    ...
    let tableInfo = this.generateTable(
      this.state.array,
      this.state.depth,
      this.state.height,
    );
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <Icon type="plus" style={{ position: 'relative', top: 1 }} />添加卡片
            </span>
            <hr style={{ marginTop: 12, color: 'white', opacity: '1.0' }} />
            <Droppable droppableId={'0'}>
              {(provided, snapshot) => (
                <div
                  className="innerTaskColumn"
                  ref={provided.innerRef}
                  style={{
                    ...snapshot.isDraggingOver,
                    minHeight: 376,
          ...
          <div
            ...
            style={{ overflow: 'auto', position: 'relative', marginTop: 2 }}
            ...
            >
              ...
              <tbody
                style={{
                  ...
                  borderCollapse: 'collapse',
                  top:
                    document.getElementsByClassName('kanban-thead')[0] != null
                      ? document.getElementsByClassName('kanban-thead')[0]
                          .offsetHeight
                      : 0,
                }}
          ...
          />
          <ReleasePlanModel
            visible={this.state.releasePlanState}
            ChangeReleasePlanState={this.ChangeReleasePlanState}
            kanbanId={this.props.match.params.kanbanId}
            getIssue={this.getIssue}
            Cards={this.state.cards['temp']}
          />
          ...
          <div style={{ display: this.state.CreateCardState }}>
            <CreateCard
              ChangeCreateCardState={this.ChangeCreateCardState}
              kanbanName={this.state.kanbanName}
              getIssue={this.getIssue}
            />
          </div>
