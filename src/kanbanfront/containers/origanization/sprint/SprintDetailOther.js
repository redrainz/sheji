...
class SprintDetailOther extends Component {
  ...
  formBaseData = value => {
    ...
    if (value.issue)
      for (let j = 0; j < value.issue.length; j += 1) {
        ...
        console.log(data.issue[j].issueId, data.issue[j].storyPoint);
        data.storyPoint +=
          data.issue[j].storyPoint == undefined ? 0 : data.issue[j].storyPoint;
        data.workload +=
          data.issue[j].workload == undefined ? 0 : data.issue[j].workload;
  ...
  getDiffSprintData = filter => {
    const list = this.state.lists;
    let filterList = [];
    filterList =
      filter === 'All' ? list : list.filter(one => one.status === filter);
    this.setState({
      issues: filterList,
  ...
  columns = [
    ...
    },
    {
      ...
      render: (text, record) => (
        <div
          style={{ ...styles.tableitem, ...{ width: '100px' } }}
          title={text}
        >
    ...
    //   sorter: (a, b) => {},
    //   render: (text, record) => (
    //     <div style={{ ...styles.tableitem, ...{ width: '50px' } }}>
    //       {text ? (text.length > 10 ? text.substring(0, 10) : text) : '-'}
    //     </div>
    //   ),
    // },
              ...
              >
                <TabPane tab="用户故事与任务" key="1">
                  <div style={{ padding: '0 24px' }}>
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            ...
                            <Option value="pre todo">pre todo</Option>
                    ...
                    >
                      <EditDetail
                        id={SprintStore.currentId}
                        init={this.init}
                        hideRight={this.hideRight.bind(this)}
                      />
                    </div>
                    <div
                      style={{
