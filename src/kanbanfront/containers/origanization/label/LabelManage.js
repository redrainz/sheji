...
class LabelManage extends Component {
  ...
  findByProjectId() {
    LabelManageStore.findByProjectId(1).then(data => {
      this.setState({
        labels: data,
      });
    });
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        key: 'look',
        render: id => (
          <a onClick={this.toManage.bind(this, id)}>查看此类型的issue</a>
        ),
      },
      {
        title: '操作',
        ...
        render: label =>
          label.projectId != 0 ? (
            <div>
              <Icon
                type="edit"
                style={{ fontSize: 16, marginRight: 24, cursor: 'pointer' }}
                onClick={this.toEdit.bind(this, label.id)}
              />
              <Icon
                type="delete"
                style={{ fontSize: 16, marginRight: 24, cursor: 'pointer' }}
                onClick={this.showDeleteConfirm.bind(this, label.id)}
              />
            </div>
          ) : (
            <div />
